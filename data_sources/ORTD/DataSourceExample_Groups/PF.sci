...
function [PacketFramework,SourceID] = ld_PF_addsource(PacketFramework, NValues_send, datatype, SourceName)
    ...
    Source.Group = PacketFramework.ActiveGroup;
...
function [sim, PacketFramework] = ld_PF_InitInstance(sim, InstanceName, Configuration) // PARSEDOCU_BLOCK
    ...
    //   Configuration.SenderId (optional)
    //   Configuration.debugmode (bool, optional)
    // 
    // 
    // Example:
    // 
    ...
    //   Configuration.SenderId = 12;
    //   Configuration.debugmode = %f;
    ...
    try 
        PacketFramework.Configuration.debugmode = Configuration.debugmode;
    catch
        null;
    end
    ...
    PacketFramework.SenderID = 0; // default value
    try
        PacketFramework.SenderID = Configuration.SenderId;
    catch
        null;
    end
...
function [sim, PacketFramework, Group] = ld_PF_NewGroup(sim, PacketFramework, GroupName, opt)
    ...
    PacketFramework.GroupList( Group.ID ) = Group;
...
function [sim] = ld_PF_SendGroupFinshUDP(sim, PacketFramework, GroupID)
    ...
    // Sender ID
    [sim, SenderID] = ld_const(sim, 0, PacketFramework.SenderID); // 
    [sim, SenderID_int32] = ld_ceilInt32(sim, 0, SenderID);
...
function ld_PF_Export_js(PacketFramework, fname) // PARSEDOCU_BLOCK
    ...
    mfprintf(fd,' ""SenderID"" : ""%s"", \n', string(PacketFramework.SenderID) );
    ...
    for i=1:length(PacketFramework.Sources)
        ...
        line=sprintf(" ""%s"" : { ""SourceName"" : ""%s"" , ""NValues_send"" : ""%s"", ""datatype"" : ""%s"", ""GroupID"" : ""%s""  } \n", ...
        ...
        string(PacketFramework.Sources(i).datatype), string(PacketFramework.Sources(i).Group.ID) );
    ...
    // Section ParametersConfig
    mfprintf(fd,' ""ParametersConfig"" : {\n');
    ...
    mfprintf(fd,' ""GroupsConfig"" : {\n');

    // go through all parameters and create memories for all
    for i=1:length(PacketFramework.GroupList)

        printf("export of Group %s \n",PacketFramework.GroupList(i).Name );

        line=sprintf(" ""%s"" : { ""Name"" : ""%s""  } \n", ...
        string(PacketFramework.GroupList(i).ID) , ...
        string(PacketFramework.GroupList(i).Name) ...
        );


        if i==length(PacketFramework.GroupList) 
            // finalise the last entry without ","
            printf('%s \n' , line);
            mfprintf(fd,'%s', line);
        else
